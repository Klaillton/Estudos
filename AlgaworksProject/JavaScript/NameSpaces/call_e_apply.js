var AlgaWorks = AlgaWorks || {}; /*Se já existir um objeto com aquele
nome, use-o, caso contrário use um objeto vazio*/
...
AlgaWorks.Cidade = (function(){
	...
	Cidade.prototype.iniciar = function(){
		//Attachar (handler) quando o estado for alterado
	...
	function onEstadoSelecionado(evento, uf){
		if(uf){
			...
			reset.call(this);
			//reset.apply(this); usado quando os parametros pra usar estão em array
		}
	}
	...
	function onCidadesRetornadas(cidades){
		...
		cidades.forEach(function(cidade){
			var optionCidade = 
			$('<option>').val(cidade.codigo).text(cidade.nome);
